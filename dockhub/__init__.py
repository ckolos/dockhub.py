...
def get_auth_token():
    ...
    if not username:
        die(
            "For security, you must set your username as the variable DH_USERNAME in your ENV"
        )

    password = getenv("DH_PASSWORD", "").strip()
    if not password:
        die(
            "For security, you must set your password as the variable DH_PASSWORD in your ENV"
        )
    ...
    if auth_request.status_code == 200:
        ...
        die(
            "Non-200 response received from DockerHub. Verify your credentials and try again"
        )
...
def add_user_to_group(auth_header, dh_user, dh_group, group_id):
    ...
    add_user = requests.post(
        group_members_url, headers=additional_headers, json=add_user_json
    )
    if add_user.status_code == 200:
        ...
        if members.status_code == 200:
            ...
            die(
                f"Could not verify {dh_user} has been added to {dh_group}. Caveat emptor!"
            )
    else:
...
def dump_user_info(auth_header, dh_user):
    ...
    if user.status_code == 200:
        ...
        die(
            "Non-200 response form DockerHub. Verify the specified user name and try agin"
        )
...
def remove_user_from_group(auth_header, dh_user, dh_group):
    ...
    if removed_user.status_code == 204:
        ...
        if members.status_code == 200:
            ...
            die(
                f"Could not verify {dh_user} has been removed from {dh_group}. Caveat emptor!"
            )
    else:
...
def add_group_to_repo(auth_header, dh_group, dh_repo, group_id):
    ...
    add_group = requests.post(
        docker_api_url, headers=additional_headers, json=add_group_json
    )
...
def dump_repo_info(auth_header, dh_repo):
    ...
    if repo.status_code == 200 and repo_perms.status_code == 200:
        ...
        die(
            "Non-200 response form DockerHub. Verify the specified repo name and try agin"
        )
...
@click.option(
    ...
    help="What action to take: add, list (default), or remove",
    required=False,
)
@click.option(
    "-f",
